...
function QSM= admm_qsm_withMask(phase_use, N,mask_use, spatial_res,lambda)
...
%     x = ifftn( (mu * (tx + ty + tz) + Dt_kspace) ./ (eps + K2 + mu * E2) );
    ...
    A=@(xx)(L2_term(kernel,mask_use,E2,mu, xx) );
    cg_tol = 1e-4;%tol*norm(E_noise_level( Mask(:) == 1))/norm(b(:));
    itermax = 20;
    [x,~,~] = cgsolve(A, b, cg_tol, itermax, 1);
    x = reshape(x,N);
