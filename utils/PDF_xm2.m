...
function [RDF, back_PDF, shim] = PDF_xm2(iFreq, Masks, Maskl, matrix_size,voxel_size, B0_dir, itermax)
...
Maskl=Maskl(d1first:d1last,d2first:d2last, d3first:d3last);
Masks=Masks(d1first:d1last,d2first:d2last, d3first:d3last);
...
Maskl = padarray(Maskl, padsize, 0,'post');
Masks = padarray(Masks, padsize, 0,'post');
...
p_final = (iFreq-p_dipole).*Masks;  
p_back = p_dipole.*Masks;
